function dydt = sys_shut_fault(t,y)


%% Shutdown model for actuator fault!
...
Pitch = pitch_initial;
C_q_fault =  C_q_Fit(1) + C_q_Fit(2).*Pitch + C_q_Fit(3).*TSR + C_q_Fit(4).*Pitch.^2 +C_q_Fit(5).*Pitch.*TSR + C_q_Fit(6).*TSR.^2;
C_t_fault = C_t_Fit(1) + C_t_Fit(2).*Pitch + C_t_Fit(3).*TSR + C_t_Fit(4).*Pitch.^2 +C_t_Fit(5).*Pitch.*TSR + C_t_Fit(6).*TSR.^2;

%C_t = polyval2(C_t_Fit,Pitch,TSR);


F_aero = 0*0.5*rho*A*v_w_eff^2*(2/3*C_t + C_t_fault/3);
Tau_aero = 0.5*rho*A*R*v_w_eff^2*(2/3*C_q + C_q_fault/3);
